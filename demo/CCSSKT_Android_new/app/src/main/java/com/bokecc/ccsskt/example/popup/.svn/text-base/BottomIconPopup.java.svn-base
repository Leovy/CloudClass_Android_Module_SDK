...
public class BottomIconPopup extends BasePopupWindow {
    ...
    private static final int resIds[] = new int[]{
            R.drawable.auth_draw_normal, /*R.drawable.lock_normal,*/ R.drawable.close_camera_normal,
            R.drawable.close_mic_normal, R.drawable.kickout_normal, R.drawable.video_fullscreen_normal
    };
    private static final String resDess[] = new String[]{
            "授权标注", /*"不轮播",*/ "关闭视频", "关麦", "踢下麦", "全屏视频"
    };
...
//        }
        ...
        if (CCInteractSession.getInstance().getTemplate() == CCInteractSession.TEMPLATE_TILE || position == -1) {
            if (haveVideo) {
                remove(mIconEntities.size()-1);
                haveVideo = false;
            }
            if(mIconEntities.size() == 5) {
                remove(mIconEntities.size()-1);
            }
        } else {
            if (!haveVideo) {
                if (CCInteractSession.getInstance().getTemplate() == CCInteractSession.TEMPLATE_SPEAK) {
                    ...
                    add(mIconEntities.size(), new IconEntity(R.drawable.video_fullscreen_normal, "全屏视频"));
                } else if(CCInteractSession.getInstance().getTemplate() == CCInteractSession.TEMPLATE_SINGLE){
                    add(mIconEntities.size(), new IconEntity(R.drawable.video_fullscreen_normal, "主视频"));
                }
            } else {
                if (CCInteractSession.getInstance().getTemplate() == CCInteractSession.TEMPLATE_SPEAK) {
                    update(mIconEntities.size()-1, new IconEntity(R.drawable.video_fullscreen_normal, "全屏视频"));
                } else if (CCInteractSession.getInstance().getTemplate() == CCInteractSession.TEMPLATE_SINGLE){
                    update(mIconEntities.size()-1, new IconEntity(R.drawable.video_fullscreen_normal, "主视频"));
                }
            }
        }
        if(mIconEntities.size() > 5) {
